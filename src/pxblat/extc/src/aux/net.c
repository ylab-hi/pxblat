 ...
 * Complain and return something negative if can't. */
{
  ...
  if (sd < 0) warn("Couldn't make %s socket.", familyToString(address->ai_family));
...
int netConnectWithTimeoutOneAddr(int sd, struct addrinfo *address, long msTimeout, char *hostName, int port,
                                 struct dyString *dy)
  ...
  if (res < 0) {
    if (errno == EINPROGRESS) {
      ...
      while (1) {
        ...
        FD_SET(sd, &mySet);
        exceptSet = mySet;
        // use tempTime (instead of using remainingTime directly) because on some platforms select() may modify the time
        // val.
        ...
        if (res < 0) {
          ...
          if (valOpt) {
            dyStringPrintf(dy, "Error in TCP non-blocking connect() %d - %s. Host %s IP %s port %d.\n", valOpt,
                           strerror(valOpt), hostName, ipStr, port);
            return -1;
          }
          break;  // OK
        } else {
          dyStringPrintf(
              dy, "TCP non-blocking connect() to %s IP %s timed-out in select() after %ld milliseconds - Cancelling!",
              hostName, ipStr, msTimeout);
          return -1;
        }
      }
    } else {
 ...
 * Also closes sd if error. */
{
  ...
  if (!internetGetAddrInfo6n4(hostName, portStr, &addressList)) return -1;

  struct dyString *errMsg = dyStringNew(256);
  for (address = addressList; address; address = address->ai_next) {
    if ((sd = netStreamSocketFromAddrInfo(address)) < 0) continue;

    if (netConnectWithTimeoutOneAddr(sd, address, msTimeout, hostName, port, errMsg) == 0) break;
...
{
  if (!isdigit(portName[0])) errAbort("netConnectTo: ports must be numerical, not %s", portName);
...
{
  FILE *f;
  if ((socket = dup(socket)) < 0) errnoAbort("Couldn't dupe socket in netFileFromSocket");
  f = fdopen(socket, "r+");
  if (f == NULL) errnoAbort("Couldn't fdopen socket in netFileFromSocket");
  return f;
}
 ...
 * Don't give up on first read! */
{
  ...
  if (!plumberInstalled) netBlockBrokenPipes();
  while (totalRead < size) {
    oneRead = read(sd, buf + totalRead, size - totalRead);
    if (oneRead < 0) return oneRead;
    if (oneRead == 0) break;
    totalRead += oneRead;
  }
  return totalRead;
 ...
 * Don't give up if first read is short! */
{
  ...
  if (ret < 0) errnoAbort("Couldn't finish netReadAll");
  return ret;
}
  ...
  *rangeEnd = -1;
  ...
  if (x) {
    if (startsWith(";byterange=", x)) {
      ...
      if (z) {
        ++z;
        if (terminateAtByteRange) *x = 0;
        *rangeStart = atoll(y);
        if (z[0] != '\0') *rangeEnd = atoll(z);
      }
    }
  }
}
 ...
 */
{
  ...
  if (strlen(url) >= sizeof(buf)) errAbort("Url too long: '%s'", url);
  strcpy(buf, url);
  url = buf;
  ...
  if (u == NULL)
    ...
    if (sameWord(parsed->protocol, "http") || sameWord(parsed->protocol, "https")) {
  ...
  if ((brCount != blCount) || (brCount > 1)) errAbort("badly formed url, stray square brackets in IPv6 address");
  ...
  if (br) {
    // expecting *s == [
    if (*s != '[') errAbort("badly formed url %s, expected [ at start of ipv6 address", s);
    ++s;      // skip [
    *br = 0;  // erase ]
    t = br + 1;
    char c = *t;
  ...
  {
    *t++ = 0;
    if (!isdigit(t[0])) errAbort("Non-numeric port name %s", t);
    safecpy(parsed->port, sizeof(parsed->port), t);
  } else  // get default port for each protocol
  {
    if (sameWord(parsed->protocol, "http")) strcpy(parsed->port, "80");
    if (sameWord(parsed->protocol, "https")) strcpy(parsed->port, "443");
    if (sameWord(parsed->protocol, "ftp")) strcpy(parsed->port, "21");
  }
...
/* Build URL from netParsedUrl structure */
{
  ...
  if (!((sameString(npu->protocol, "ftp") && sameString("21", npu->port)) ||
  ...
  if (npu->byteRangeStart != -1) {
    ...
    if (npu->byteRangeEnd != -1) dyStringPrintf(dy, "%lld", (long long)npu->byteRangeEnd);
  }
 ...
 * warn and return FALSE if not desired reply.  If retReply is non-NULL, store reply text there. */
{
  ...
  while (1) {
    ...
    while (1) {
      ...
      if ((readSize = read(sd, buf, sizeof(buf))) == 0) break;
    ...
    if (strlen(startLastLine) > 4)
      if (isdigit(startLastLine[0]) && isdigit(startLastLine[1]) && isdigit(startLastLine[2]) &&
          startLastLine[3] == ' ')
        break;
...
/* parse PASV reply to get the port and return it */
{
  ...
  *rsEnd = 0;
  ...
  if (wordCount != 6) errAbort("PASV reply does not parse correctly");
...
/* parse EPSV reply to get the port and return it */
{
  ...
  *rsEnd = 0;
  wordCount = chopString(rsStart, "|", words, ArraySize(words));
  if (wordCount != 1) errAbort("EPSV reply does not parse correctly");
...
/* parse reply to SIZE and return it */
{
  ...
  long long result = 0;
  wordCount = chopString(rsStart, " ", words, ArraySize(words));
  if (wordCount != 2) errAbort("SIZE reply does not parse correctly");
 ...
 * 200 YYYYMMDDhhmmss */
{
  ...
  if (wordCount != 2) errAbort("MDTM reply should have 2 words but has %d", wordCount);
#define EXPECTED_MDTM_FORMAT "YYYYmmddHHMMSS"
  ...
  *to++ = 0;
  struct tm tm;
  if (strptime(spread, "%Y-%m-%d %H:%M:%S", &tm) == NULL) errAbort("unable to parse MDTM string [%s]", spread);
  // Not set by strptime(); tells mktime() to determine whether daylight saving time is in effect:
  tm.tm_isdst = -1;
  time_t t = mktime(&tm);
  if (t == -1) errAbort("mktime failed while parsing strptime'd MDTM string [%s]", words[1]);
  return t;
}
 ...
 * return socket desc or -1 if there was an error. */
{
  ...
  if (readReadyWait(sd, NET_FTP_TIMEOUT)) sendFtpCommand(sd, "", NULL, NULL);
...
/* Return date in UTC and size of ftp url file */
{
  ...
  netParseUrl(url, &npu);
  if (!sameString(npu.protocol, "ftp")) errAbort("netGetFtpInfo: url (%s) is not for ftp.", url);
 ...
 * and reads from the pipe. */
{
  ...
  int rd = 0;
  long long dataPos = 0;
  if (params->npu.byteRangeStart != -1) dataPos = params->npu.byteRangeStart;
  while ((rd = read(params->sdata, buf, 32768)) > 0) {
    ...
    dataPos += rd;
    if (params->npu.byteRangeEnd != -1 && dataPos >= params->npu.byteRangeEnd) break;
  }
  if (rd == -1)
 ...
 * process until we are done fetching data. */
{
  ...
  netParseUrl(url, &npu);
  if (!sameString(npu.protocol, "ftp")) errAbort("netGetOpenFtpSockets: url (%s) is not for ftp.", url);

  boolean noProxy = checkNoProxy(npu.host);
  char *proxyUrl = getenv("ftp_proxy");
  if (noProxy) proxyUrl = NULL;
  ...
  if (proxyUrl) {
    ...
    if (sameOk(logProxy, "on")) verbose(1, "%s as %s via proxy %s\n", url, proxyUser, proxyUrl);
  } else {
  ...
  while (TRUE) {
    ...
    if (readReadyWait(sdata, NET_FTP_TIMEOUT)) break;  // we have some data
  ...
  if (retCtrlSd != NULL) {
    *retCtrlSd = sd;
    ...
    if (pipe(params->pipefd) != 0) errAbort("netGetOpenFtpSockets: failed to create pipe: %s", strerror(errno));
    int rc;
...
/* See if host endsWith element on no_proxy list. Elements are comma-separated. */
{
  ...
  while ((word = nextWord(&list))) {
    if (endsWith(host, word)) return TRUE;
  }
  return FALSE;
}
 ...
 * Cert verify domains exception white-list via hg.conf httpsCertCheckDomainExceptions or env var
 * https_cert_check_domain_exceptions Return data socket, or -1 if error.*/
{
  struct netParsedUrl npu;
  struct netParsedUrl pxy;
  struct dyString *dy = dyStringNew(512);
  ...
  if (sameString(npu.protocol, "https")) proxyUrl = NULL;
  if (noProxy) proxyUrl = NULL;
  if (proxyUrl) {
    netParseUrl(proxyUrl, &pxy);
    if (!sameString(pxy.protocol, "http")) errAbort("Unknown proxy protocol %s in %s.", pxy.protocol, proxyUrl);
    sd = connectNpu(pxy, url, noProxy);
    char *logProxy = getenv("log_proxy");
    if (sameOk(logProxy, "on")) verbose(1, "%s via proxy %s\n", url, proxyUrl);
  } else {
  ...
  if (proxyUrl) {
    ...
    if (x && startsWith(";byterange=", x)) *x = 0;
  }
  ...
  if (sameString(npu.protocol, "http") && sameString("80", npu.port)) portIsDefault = TRUE;
  if (sameString(npu.protocol, "https") && sameString("443", npu.port)) portIsDefault = TRUE;
  if (!portIsDefault) {
    dyStringAppendC(dy, ':');
    dyStringAppend(dy, npu.port);
  }
  ...
  if (proxyUrl) setAuthorization(pxy, "Proxy-Authorization", dy);
  dyStringAppend(dy, "Accept: */*\r\n");
  if (npu.byteRangeStart != -1) {
    if (npu.byteRangeEnd != -1)
      dyStringPrintf(dy, "Range: bytes=%lld-%lld\r\n", (long long)npu.byteRangeStart, (long long)npu.byteRangeEnd);
    else
      dyStringPrintf(dy, "Range: bytes=%lld-\r\n", (long long)npu.byteRangeStart);
  }

  if (optionalHeader) dyStringAppend(dy, optionalHeader);
...
/* Slurp file into dynamic string and return. */
{
  ...
  while ((readSize = read(sd, buf, sizeof(buf))) > 0) dyStringAppendN(dy, buf, readSize);
  return dy;
}
...
/* Go grab all of URL and return it as dynamic string. */
{
  ...
  if (sd < 0) errAbort("netSlurpUrl: failed to open socket for [%s]", url);
 ...
 * the new location. */
{
  ...
  while (TRUE) {
    ...
    if (startsWith("HTTP/", line)) {
      ...
      if (sameString(code, "301") || sameString(code, "302") || sameString(code, "307") || sameString(code, "308")) {
        ...
        if (!sameString(code, "206")) {
          if (sameString(code, "200")) warn("Byte-range request was ignored by server. ");
          warn("Expected Partial Content 206. %s: %s %s. rangeStart=%lld rangeEnd=%lld", url, code, line,
               (long long)byteRangeStart, (long long)byteRangeEnd);
          return FALSE;
        }
    ...
    if (sameWord(headerName, "Location")) {
      if (redirect) *redirectedUrl = cloneString(headerVal);
      if (mustUseProxy) proxyLocation = cloneString(headerVal);
    }
    if (sameWord(headerName, "Content-Range")) {
      if (byteRangeUsed) {
        foundContentRange = TRUE;
        parseContentRange(headerVal, &contentRangeStart, &contentRangeEnd);
        if ((contentRangeStart != byteRangeStart) || (byteRangeEnd != -1 && (contentRangeEnd != byteRangeEnd))) {
          char bre[256];
          safef(bre, sizeof bre, "%lld", (long long)byteRangeEnd);
          if (byteRangeEnd == -1) bre[0] = 0;
  ...
  if (mustUseProxy || mustUseProxyAuth) {
    warn("%s: %s error. Use Proxy%s. Location = %s", url, mustUseProxy ? "" : " Authentication",
         mustUseProxy ? "305" : "407", proxyLocation ? proxyLocation : "not given");
    return FALSE;
  }
  ...
  while (TRUE) {
    ...
    if (success) {
      ...
      if (redirectCount > 5) {
        ...
        if (!startsWith("http://", newUrl) && !startsWith("https://", newUrl)) {
    ...
    if (redirectCount > 1) freeMem(url);
    if (!success) { /* failure after 0 to 5 redirects */
      if (redirectCount > 0) freeMem(newUrl);
      return FALSE;
    }
    url = newUrl;
  }
 ...
 * does not abort, just returning NULL if there's a problem. */
{
  ...
  if (sd < 0) {
    ...
    boolean isCompressed = (endsWith(urlDecoded, ".gz") || endsWith(urlDecoded, ".Z") || endsWith(urlDecoded, ".bz2"));
    freeMem(urlDecoded);
    if (isCompressed) {
 ...
 * return from http after forwarding. */
{
  ...
  if (startsWith("http://", url) || startsWith("https://", url)) {
    char *newUrl = NULL;
    if (!netSkipHttpHeaderLinesHandlingRedirect(sd, url, &newSd, &newUrl)) noWarnAbort();
    if (newUrl != NULL) {
      sd = newSd;
      freeMem(newUrl);
    }
...
/* Parse URL and open an HTTP socket for it but don't send a request yet. */
{
  ...
  AllocVar(*npu);
  netParseUrl(url, *npu);
  if (!sameString((*npu)->protocol, "http")) errAbort("netHttpLineFileMayOpen: url (%s) is not for http.", url);
  sd = netConnect((*npu)->host, atoi((*npu)->port));
  if (sd < 0) return NULL;
...
void netHttpGet(struct lineFile *lf, struct netParsedUrl *npu, boolean keepAlive)
...
int netHttpGetMultiple(char *url, struct slName *queries, void *userData,
                       void (*responseCB)(void *userData, char *req, char *hdr, struct dyString *body))
