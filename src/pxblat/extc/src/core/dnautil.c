...
struct codonTable codonTable[] =
    ...
    {
        {"ttt", 'F', 'F', 'a'}, {"ttc", 'F', 'F', 'b'}, {"tta", 'L', 'L', 'c'}, {"ttg", 'L', 'L', 'd'},

        {"tct", 'S', 'S', 'e'}, {"tcc", 'S', 'S', 'f'}, {"tca", 'S', 'S', 'g'}, {"tcg", 'S', 'S', 'h'},

        {"tat", 'Y', 'Y', 'i'}, {"tac", 'Y', 'Y', 'j'}, {"taa", 0, 0, 'k'},     {"tag", 0, 0, 'l'},

        {"tgt", 'C', 'C', 'm'}, {"tgc", 'C', 'C', 'n'}, {"tga", 0, 'W', 'o'},   {"tgg", 'W', 'W', 'p'},

        {"ctt", 'L', 'L', 'q'}, {"ctc", 'L', 'L', 'r'}, {"cta", 'L', 'L', 's'}, {"ctg", 'L', 'L', 't'},

        {"cct", 'P', 'P', 'u'}, {"ccc", 'P', 'P', 'v'}, {"cca", 'P', 'P', 'w'}, {"ccg", 'P', 'P', 'x'},

        {"cat", 'H', 'H', 'y'}, {"cac", 'H', 'H', 'z'}, {"caa", 'Q', 'Q', 'A'}, {"cag", 'Q', 'Q', 'B'},

        {"cgt", 'R', 'R', 'C'}, {"cgc", 'R', 'R', 'D'}, {"cga", 'R', 'R', 'E'}, {"cgg", 'R', 'R', 'F'},

        {"att", 'I', 'I', 'G'}, {"atc", 'I', 'I', 'H'}, {"ata", 'I', 'M', 'I'}, {"atg", 'M', 'M', 'J'},

        {"act", 'T', 'T', 'K'}, {"acc", 'T', 'T', 'L'}, {"aca", 'T', 'T', 'M'}, {"acg", 'T', 'T', 'N'},

        {"aat", 'N', 'N', 'O'}, {"aac", 'N', 'N', 'P'}, {"aaa", 'K', 'K', 'Q'}, {"aag", 'K', 'K', 'R'},

        {"agt", 'S', 'S', 'S'}, {"agc", 'S', 'S', 'T'}, {"aga", 'R', 0, 'U'},   {"agg", 'R', 0, 'V'},

        {"gtt", 'V', 'V', 'W'}, {"gtc", 'V', 'V', 'X'}, {"gta", 'V', 'V', 'Y'}, {"gtg", 'V', 'V', 'Z'},

        {"gct", 'A', 'A', '1'}, {"gcc", 'A', 'A', '2'}, {"gca", 'A', 'A', '3'}, {"gcg", 'A', 'A', '4'},

        {"gat", 'D', 'D', '5'}, {"gac", 'D', 'D', '6'}, {"gaa", 'E', 'E', '7'}, {"gag", 'E', 'E', '8'},

        {"ggt", 'G', 'G', '9'}, {"ggc", 'G', 'G', '0'}, {"gga", 'G', 'G', '@'}, {"ggg", 'G', 'G', '$'},
};
...
static void initNtVal() {
  if (!inittedNtVal) {
    ...
    ntVal5['t'] = ntVal5['T'] = ntValNoN['t'] = ntValNoN['T'] = ntVal['t'] = ntVal['T'] = ntValLower['t'] =
        ntValUpper['T'] = T_BASE_VAL;
    ntVal5['u'] = ntVal5['U'] = ntValNoN['u'] = ntValNoN['U'] = ntVal['u'] = ntVal['U'] = ntValLower['u'] =
        ntValUpper['U'] = U_BASE_VAL;
    ntVal5['c'] = ntVal5['C'] = ntValNoN['c'] = ntValNoN['C'] = ntVal['c'] = ntVal['C'] = ntValLower['c'] =
        ntValUpper['C'] = C_BASE_VAL;
    ntVal5['a'] = ntVal5['A'] = ntValNoN['a'] = ntValNoN['A'] = ntVal['a'] = ntVal['A'] = ntValLower['a'] =
        ntValUpper['A'] = A_BASE_VAL;
    ntVal5['g'] = ntVal5['G'] = ntValNoN['g'] = ntValNoN['G'] = ntVal['g'] = ntVal['G'] = ntValLower['g'] =
        ntValUpper['G'] = G_BASE_VAL;
...
/* Return TRUE if it's a Kozak compatible start. */
{
  ...
  if (pos + 3 < dnaSize) {
    if (ntVal[(int)dna[pos + 3]] == G_BASE_VAL) return TRUE;
  }
  if (pos >= 3) {
    int c = ntVal[(int)dna[pos - 3]];
    if (c == A_BASE_VAL || c == G_BASE_VAL) return TRUE;
  }
  return FALSE;
}
 ...
 * Returns -1 if not a codon. */
{
  ...
  if ((v1 = ntVal[(int)start[0]]) < 0) return -1;
  if ((v2 = ntVal[(int)start[1]]) < 0) return -1;
  if ((v3 = ntVal[(int)start[2]]) < 0) return -1;
  return ((v1 << 4) + (v2 << 2) + v3);
}
 ...
 * whichever comes first. Output will be zero terminated. */
{
  ...
  for (i = 0; i < dnaSize - 2; i += 3) {
    if (protSize >= outSize) break;
    if ((out[protSize++] = lookupCodon(dna + i)) == 0) break;
  }
  out[protSize] = 0;
}
...
long reverseOffset(long offset, long arraySize) { return arraySize - 1 - offset; }
 ...
 * 2. If alleleStr begins with "-/", then after reversing, move "-/" back to the beginning. */
{
  ...
  for (i = alCount - 1; i >= 0; i--) {
    ...
    if (isAllNt(allele, alLen)) reverseComplement(allele, alLen);
    if (i != alCount - 1) safecat(outStr, len + 1, "/");
...
/* Compare using screwy non-alphabetical DNA order TCGA */
{
  for (;;) {
    DNA aa = *a++;
    DNA bb = *b++;
    if (aa != bb) return ntVal[(int)aa] - ntVal[(int)bb];
    if (aa == 0) break;
  }
  return 0;
}
 ...
 * results in histogram. */
{
  ...
  while (--dnaSize >= 0) {
    if ((val = ntVal[(int)*dna++]) >= 0) ++histogram[val];
  }
}
...
{
  if (size > 32) errAbort("basesToBits64 called on %d bases, can only go up to 32", size);
  bits64 result = 0;
  int i;
  for (i = 0; i < size; ++i) {
    result <<= 2;
...
{
  if (iEnd - iStart < minIntronSize) return 0;
...
/* Compare two sequences (without inserts or deletions) and score. */
{
  ...
  for (i = 0; i < size; ++i) {
    char aa = a[i];
    char bb = b[i];
    if (aa == ignore || bb == ignore) continue;
    if (aa == bb)
      score += matchScore;
    else
      score += mismatchScore;
...
/* Write out letters with newlines every maxLine. */
{
  ...
  while (lettersLeft > 0) {
    ...
    if (lineSize > maxPerLine) lineSize = maxPerLine;
 ...
 * Leaves first two bases of PolyA in case there's a taa stop codon. */
{
  ...
  if (bestPos >= 0) {
    ...
    if (trimSize > 0) {
      if (doMask)
        for (i = size - trimSize; i < size; ++i) dna[i] = 'n';
    } else
      trimSize = 0;
  }
  return trimSize;
 ...
 * two tt for revcomp'd taa stop codon. */
{
  ...
  if (bestPos >= 0) {
    ...
    if (trimSize > 0) {
      if (doMask) memset(dna, 'n', trimSize);
    } else
      trimSize = 0;
  }
  return trimSize;
...
/* Return TRUE if letters in poly are at least 90% ACGTNU- */
{
  ...
  dnaUtilOpen();
  for (i = 0; i < size; ++i) {
    if (ntChars[(int)poly[i]]) dnaCount += 1;
  }
...
/* Return TRUE if all letters in seq are ACGTNU-. */
{
  ...
  dnaUtilOpen();
  for (i = 0; i < size; ++i) {
    if (ntChars[(int)seq[i]] == 0) return FALSE;
  }
  return TRUE;
}
...
struct aminoAcidTable aminoAcidTable[] = {
    {0, 'A', "ala", "alanine"},       {1, 'C', "cys", "cysteine"},      {2, 'D', "asp", "aspartic acid"},
    {3, 'E', "glu", "glutamic acid"}, {4, 'F', "phe", "phenylalanine"}, {5, 'G', "gly", "glycine"},
    {6, 'H', "his", "histidine"},     {7, 'I', "ile", "isoleucine"},    {8, 'K', "lys", "lysine"},
    {9, 'L', "leu", "leucine"},       {10, 'M', "met", "methionine"},   {11, 'N', "asn", "asparagine"},
    {12, 'P', "pro", "proline"},      {13, 'Q', "gln", "glutamine"},    {14, 'R', "arg", "arginine"},
    {15, 'S', "ser", "serine"},       {16, 'T', "thr", "threonine"},    {17, 'V', "val", "valine"},
    {18, 'W', "trp", "tryptophan"},   {19, 'Y', "tyr", "tyrosine"},     {20, 'X', "ter", "termination"},
};
...
/* Initialize aaVal and valToAa tables. */
{
  ...
  for (i = 0; i < ArraySize(aaVal); ++i) aaVal[i] = -1;
 ...
 * such as "A", "D" etc.  Return the null char '\0' if abbr is not found. */
{
  ...
  for (ix = 0; ix < ArraySize(aminoAcidTable); ix++) {
    if (sameStringN(abbrLC, aminoAcidTable[ix].abbreviation, 3)) return aminoAcidTable[ix].letter;
  }
  return '\0';
}
...
void trimRefAltDir(char *ref, char *alt, uint *pStart, uint *pEnd, int *pRefLen, int *pAltLen, boolean leftJustify)
/* If ref and alt have identical bases at beginning and/or end, trim those & update all params. */
{
  ...
  if (leftJustify) {
    ...
    while (ref[trimStart] != '\0' && alt[trimStart] != '\0' && ref[trimStart] == alt[trimStart]) trimStart++;
  } else {
