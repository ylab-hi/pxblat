...
static void savePslx(char *chromName, int chromSize, int chromOffset, struct ffAli *ali, struct dnaSeq *tSeq,
                     struct dnaSeq *qSeq, boolean isRc, enum ffStringency stringency, int minMatch, FILE *f,
                     struct hash *t3Hash, boolean reportTargetStrand, boolean targetIsRc, struct hash *maskHash,
                     int minIdentity, boolean qIsProt, boolean tIsProt, boolean saveSeq)
 ...
 * psl format (or pslX format - if saveSeq is TRUE).  */
{
  ...
  if (maskHash != NULL) maskBits = hashMustFindVal(maskHash, tSeq->name);
  ...
  /* if (score >= minMatch) Moved to higher level */
  {
    ...
    if ((matchCount + repMatch + mismatchCount) > 0) {
      ...
      if (id >= minIdentity) {
        ...
        fprintf(f, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%c", matchCount, mismatchCount, repMatch, countNs, nInsertCount,
                nInsertBaseCount, hInsertCount, hInsertBaseCount, (isRc ? '-' : '+'));
        if (reportTargetStrand) fprintf(f, "%c", (targetIsRc ? '-' : '+'));
        fprintf(f,
                "\t%s\t%d\t%d\t%d\t"
                "%s\t%d\t%d\t%d\t%d\t",
                qSeq->name, qSeq->size, nStart, nEnd, chromName, chromSize, hStart, hEnd, ffAliCount(ali));
        for (ff = ali; ff != NULL; ff = ff->right) fprintf(f, "%ld,", (long)(ff->nEnd - ff->nStart));
        fprintf(f, "\t");
        for (ff = ali; ff != NULL; ff = ff->right) fprintf(f, "%ld,", (long)(ff->nStart - needle));
        fprintf(f, "\t");
...
static void pslOut(char *chromName, int chromSize, int chromOffset, struct ffAli *ali, struct dnaSeq *tSeq,
                   struct hash *t3Hash, struct dnaSeq *qSeq, boolean qIsRc, boolean tIsRc, enum ffStringency stringency,
                   int minMatch, struct gfOutput *out)
/* Save psl for more complex alignments. */
{
  struct pslxData *outForm = out->data;

  savePslx(chromName, chromSize, chromOffset, ali, tSeq, qSeq, qIsRc, stringency, minMatch, outForm->f, t3Hash,
           out->reportTargetStrand, tIsRc, out->maskHash, out->minGood, out->qIsProt, out->tIsProt, outForm->saveSeq);
}

static struct ffAli *ffNextBreak(struct ffAli *ff, int maxInsert, bioSeq *tSeq, struct trans3 *t3List)
 ...
 * be NULL. */
{
  ...
  for (;;) {
    ...
    if (hGap != 0 && nGap != 0) break;
    if (hGap < 0 || nGap < 0) break;
    if (hGap > maxInsert || nGap > maxInsert) break;
    ff = rt;
    rt = ff->right;
  }
  return rt;
...
static void saveAxtBundle(char *chromName, int chromSize, int chromOffset, struct ffAli *ali, struct dnaSeq *tSeq,
                          struct hash *t3Hash, struct dnaSeq *qSeq, boolean qIsRc, boolean tIsRc,
  ...
  for (sAli = ali; sAli != NULL; sAli = eAli) {
    ...
    for (ff = sAli; ff != eAli; ff = ff->right) {
      ...
      if (rt != eAli) {
        ...
        int nhStart = trans3GenoPos(rt->hStart, tSeq, t3List, FALSE) + chromOffset;
        int ohEnd = trans3GenoPos(ff->hEnd, tSeq, t3List, TRUE) + chromOffset;
        int hGap = nhStart - ohEnd;
...
/* Do axt oriented output - at end of processing query. */
{
  ...
  for (gab = aod->bundleList; gab != NULL; gab = gab->next) {
    struct axt *axt;
    for (axt = gab->axtList; axt != NULL; axt = axt->next) axtWrite(axt, f);
  }
...
/* Return matches. */
{
  ...
  for (i = 0; i < axt->symCount; ++i) {
    if (axt->qSym[i] == axt->tSym[i]) ++matchCount;
  }
  return matchCount;
}
...
/* Do sim4-like output - at end of processing query. */
{
  ...
  for (gab = aod->bundleList; gab != NULL; gab = gab->next) {
    ...
    if (!(goodPpt >= out->minGood)) continue;
    fprintf(f, "\n");
...
{
  struct axtData *aod = out->data;
  axtBlastOut(aod->bundleList, out->queryIx, out->qIsProt, f, aod->databaseName, aod->databaseSeqCount,
              aod->databaseLetters, aod->blastType, "blat", aod->minIdentity);
  axtBundleFreeList(&aod->bundleList);
}
...
/* Set up psl/pslx output */
{
  ...
  out->out = pslOut;
  out->data = pslData;
  if (!noHead) out->fileHead = pslHead;
  return out;
}
...
struct gfOutput *gfOutputMaf(int goodPpt, boolean qIsProt, boolean tIsProt, FILE *f)
...
struct gfOutput *gfOutputSim4(int goodPpt, boolean qIsProt, boolean tIsProt, char *databaseName)
/* Set up to output in sim4 format. */
{
  struct gfOutput *out = gfOutputAxtMem(goodPpt, qIsProt, tIsProt);
  struct axtData *ad = out->data;
  if (qIsProt || tIsProt) errAbort("sim4 output is not available for protein query sequences.");
...
struct gfOutput *gfOutputBlast(int goodPpt, boolean qIsProt, boolean tIsProt, char *databaseName, int databaseSeqCount,
                               double databaseLetters, char *blastType, double minIdentity, FILE *f)
 ...
 */
{
  ...
  if (sameWord(format, "psl"))
    ...
    out = gfOutputBlast(goodPpt, qIsProt, tIsProt, databaseName, databaseSeqCount, databaseLetters, format, minIdentity,
                        f);
...
{
  ++out->queryIx;
  if (out->queryOut != NULL) out->queryOut(out, f);
}

void gfOutputHead(struct gfOutput *out, FILE *f)
/* Write out header if any. */
{
  if (out->fileHead != NULL) out->fileHead(out, f);
}
